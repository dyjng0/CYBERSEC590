...
import type { ChatMessage, ChatRequest, ChatResponse, HealthResponse, SupportContext } from "../shared/types";
...
const PHONE = {
  ownerName: "Dad",
  ownerSub: "Fictional demo · iPhone 15",
  model: "iPhone 15",
  finish: "Black, 128 GB",
  os: "iOS 18.5",
  purchased: "Sep 2023"
...
function TopBar({ provider, model, status }: { provider: string; model: string; status: string }) {
  return (
    <header className="topbar">
      ...
      <span className="status">
        {provider || "Loading provider"} · {model || "loading model"} · {status}
      </span>
    </header>
  );
}
...
export function App() {
  ...
  const [tracingConfigured, setTracingConfigured] = useState(false);
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        ...
        setTracingConfigured(health.langfuseCredentialsPresent);
        if (instanceRef.current !== health.instanceId) {
          instanceRef.current = health.instanceId;
          setConnectionStatus(health.modelCredentialsPresent ? "not checked" : "API key missing");
        }
      } catch {
        if (!cancelled) setAppAvailable(false);
      }
    };
    void check();
  ...
  async function send(textOverride?: string) {
    ...
    setChatError("");
    setUsedTools([]);
    setConnectionStatus("request in progress");
    ...
    try {
      ...
      setModel(result.traceMeta.model);
      setProvider(result.traceMeta.provider);
      setUsedTools(result.usedTools);
      setConnectionStatus("last request succeeded");
      setMessages((current) => [...current, createMessage("assistant", result.answer)]);
    } catch (error) {
      setConnectionStatus("last request failed");
  ...
  return (
    <div className="app">
      <TopBar provider={provider} model={model} status={appAvailable ? connectionStatus : "app unavailable · reconnecting"} />
      <div className="lab-status" role="status">
        Langfuse keys: {tracingConfigured ? "present — this does not confirm tracing" : "missing"}
        {usedTools.length > 0 && <span> · Completed tools: {usedTools.join(", ")}</span>}
      </div>
      <div className="main">
        <section className="chat">
          ...
          {chatError && <div className="lab-error" role="alert">{chatError}</div>}
